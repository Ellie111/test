...
def read_csv():
    persons = []
    with open('kk.csv', 'rt', encoding="utf-8") as f:
        reader = csv.reader(f)
...
def export_csv(combination):
    ...
    try:
        ...
        with open("匿名化结果.csv", 'w',encoding="utf-8") as myfile:
...
def main():
    try:
        print("输入您想要的K值")
        given_k = 1 #你想要的k值int(sys.argv[1])
    except:
    ...
    while (1):
        ...
        k = 1 #得到目前的K值 get_k(grouped_persons)
        ...
        if(sex_num==1 and zip_num==1 and sex_num==1):
            break   #如果三个属性值都为1说明所有敏感属性全部泛化完成，跳出循环
        m = max_num(data_num,zip_num,sex_num)#返回属性个数最大的属性
        if m == 1:
            #如果是出生日期，对出生日期进行一位泛化
